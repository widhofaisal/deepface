...
class Image(db.Model):
    ...
    fruser = db.relationship(
        'FrUser', backref=db.backref('image', lazy='dynamic'))
...
class LevelDistance(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    identify_distance = db.Column(db.Float, nullable=False)
    verify_distance = db.Column(db.Float, nullable=False)

    def to_dict(self):
        return {
            'id': self.id,
            'identify_distance': self.identify_distance,
            'verify_distance': self.verify_distance
        }
